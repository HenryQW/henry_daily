...
async function getArrivals(req, res) {
  ...
  try {
    rail.getArrivalsBoardWithDetails(origin.toUpperCase(),
...
async function getTimeTable(req, res) {
  ...
  const trainDest = destination === 'RDG' ? ['Reading', 'London Paddington'] : ['Swansea', 'Carmarthen'];
  ...
  try {
    ...
    for (let i = 0; i < hourTo - hourFrom; i++) {
      axiosList.push(axios.get(`${api}/${hourFrom + i}:00/timetable.json`));
      axiosList.push(axios.get(`${api}/${hourFrom + i}:30/timetable.json`));
    }

    (await axios.all(axiosList)).map(f => f.data.departures.all.forEach((t) => {
      if (t.destination_name === trainDest[0] || t.destination_name === trainDest[1]) {
